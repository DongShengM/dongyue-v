  ...
  export default {
    ...
    methods: {
      ...
      async getXXTable(date) {
        ...
        let pdata = await this.$axios.post("/studentEntrance/getStudentEntrance", date)
      ...
      async getNavData() {
        try {
          let that = this
          let resdate = await this.$axios.get('/school/getSchoolInfo')
      ...
      async changejingTime() {
        ...
        await this.getkQData('/studentEntrance/selectAttendance/' + time)
        await this.getLineData('/studentEntranceStatistics/getStudentEntranceStatistics/' + time)
